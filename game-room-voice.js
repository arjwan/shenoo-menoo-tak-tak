...
function ui(){var box=document.createElement('section');box.className='game-voice';box.innerHTML='<button data-vjoin>🎙 انضم للصوت</button><button data-vmute hidden>🔇 الميكروفون</button><button data-vspeaker hidden>🔊 السماعة</button><button data-vleave hidden>✕</button><span data-vcount>0 متصل</span><small data-vmsg></small>';document.body.appendChild(box);return box}
var box=ui(),joinBtn=box.querySelector('[data-vjoin]'),muteBtn=box.querySelector('[data-vmute]'),speakerBtn=box.querySelector('[data-vspeaker]'),leaveBtn=box.querySelector('[data-vleave]'),count=box.querySelector('[data-vcount]'),message=box.querySelector('[data-vmsg]');
...
function peer(uid){uid=String(uid);if(peers.has(uid))return peers.get(uid);var pc=new RTCPeerConnection(ice);peers.set(uid,pc);if(stream)stream.getTracks().forEach(function(t){pc.addTrack(t,stream)});pc.onicecandidate=function(e){if(e.candidate)socket.emit('webrtc:voice-ice',{roomId:roomId,userId:uid,data:e.candidate})};pc.ontrack=function(e){var a=audios.get(uid);if(!a){a=document.createElement('audio');a.autoplay=true;a.playsInline=true;a.hidden=true;document.body.appendChild(a);audios.set(uid,a)}a.srcObject=e.streams[0];a.muted=!speaker};pc.onconnectionstatechange=function(){if(['failed','closed'].includes(pc.connectionState))closePeer(uid)};return pc}
...
async function join(){
 ...
 socket.on('voice:participants',function(d){if(String(d.roomId)!==String(roomId))return;var ids=(d.participants||[]).map(function(x){return String(x.id)});count.textContent=ids.length+' متصل';Array.from(peers.keys()).forEach(function(k){if(!ids.includes(k))closePeer(k)});(d.participants||[]).forEach(function(p){var uid=String(p.id);if(uid!==String(window.kahwaCurrentUserId)&&!peers.has(uid)&&String(window.kahwaCurrentUserId)<uid)offer(uid).catch(function(){})})});
...
joinBtn.onclick=join;muteBtn.onclick=function(){muted=!muted;if(stream)stream.getAudioTracks().forEach(function(t){t.enabled=!muted});muteBtn.textContent=muted?'🔇 الميكروفون':'🎙 الميكروفون مفتوح';socket&&socket.emit('voice:mute-state',{roomId:roomId,muted:muted})};speakerBtn.onclick=function(){speaker=!speaker;audios.forEach(function(a){a.muted=!speaker});speakerBtn.textContent=speaker?'🔊 السماعة':'🔈 السماعة مغلقة'};leaveBtn.onclick=leave;window.addEventListener('beforeunload',leave);
})();