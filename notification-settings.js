...
function save(p){localStorage.setItem(KEY,JSON.stringify(p));window.dispatchEvent(new CustomEvent('shno:notification-settings',{detail:p}));}
...
function install(){
 ...
 row('lockScreenPreview','إظهار محتوى الرسالة على شاشة القفل','عند إيقافه يظهر اسم شنو منو فقط حفاظاً على الخصوصية.')+
 ...
 var p=load();section.querySelectorAll('[data-notification-pref]').forEach(function(input){input.checked=!!p[input.dataset.notificationPref];});
