...
function card(n){
  const a=n.actor||{};
  const pic=a.avatarUrl?'<img src="'+esc(abs(a.avatarUrl))+'" alt="">':esc((a.fullName||a.username||'?').slice(0,1));
  const target=targetFor(n);
  const when=n.createdAt?new Date(n.createdAt).toLocaleString('ar-IQ'):'';
  return '<div class="home-notification-item '+(!n.readAt?'unread':'')+'" role="button" tabindex="0" data-notification-id="'+esc(n.id)+'" data-notification-href="'+esc(target)+'"><span class="home-notification-avatar">'+pic+'</span><span class="home-notification-copy"><strong>'+esc(a.fullName||a.username||'شنو منو')+'</strong><span class="home-notification-text">'+esc(n.text||'لديك إشعار جديد')+'</span><small>'+esc(when)+'</small></span></div>';
}
...
async function openItem(item){const target=item.dataset.notificationHref||'taktak.html';try{await api('/api/social/notifications/'+item.dataset.notificationId+'/read',{method:'PATCH'})}catch(_){}setOpen(false);location.href=target}
menu.addEventListener('click',e=>{const item=e.target.closest('[data-notification-id]');if(item)openItem(item)});
menu.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!==' ')return;const item=e.target.closest('[data-notification-id]');if(!item)return;e.preventDefault();openItem(item)});
