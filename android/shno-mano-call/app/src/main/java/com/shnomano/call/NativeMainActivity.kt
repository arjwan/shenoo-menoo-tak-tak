...
@Composable private fun ShnoApp(){
    // Screen contains custom Kotlin objects and is not Bundle-saveable. Using
    // rememberSaveable here caused an immediate startup crash on real devices.
...
@Composable private fun LoginScreen(onPreview:()->Unit){
    ...
    AppBackground{
        Column(Modifier.fillMaxSize().statusBarsPadding().padding(26.dp),horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.Center){
            ...
            Spacer(Modifier.height(12.dp));OutlinedButton(onClick=onPreview,modifier=Modifier.fillMaxWidth().height(50.dp),colors=ButtonDefaults.outlinedButtonColors(contentColor=Accent)){Text("معاينة التصميم الجديد")}
            Spacer(Modifier.height(16.dp));Text("هذه النسخة مستقلة عن الموقع. ربط الحساب والمزامنة سيتم في المرحلة التالية.",color=TextMuted,fontSize=11.sp,textAlign=TextAlign.Center)
        }
    }
}
...
@Composable private fun HomeScreen(tab:MainTab,onTab:(MainTab)->Unit,onChat:(Person)->Unit,onProfile:(Person)->Unit,onContacts:()->Unit,onStatus:()->Unit){
    AppBackground{Column(Modifier.fillMaxSize().statusBarsPadding()){TopBar(tab,onContacts,onStatus);Box(Modifier.weight(1f)){when(tab){MainTab.CHATS->ChatsScreen(onChat,onProfile,onStatus);MainTab.CALLS->CallsScreen(onProfile);MainTab.COMMUNITY->CommunityScreen(onProfile);MainTab.SETTINGS->SettingsScreen()}};BottomBar(tab,onTab)}}
}

@Composable private fun TopBar(tab:MainTab,onContacts:()->Unit,onStatus:()->Unit){
    Row(Modifier.fillMaxWidth().padding(horizontal=16.dp,vertical=9.dp),verticalAlignment=Alignment.CenterVertically){Text(if(tab==MainTab.CHATS)"شنو منو" else tab.title,color=Color.White,fontSize=26.sp,fontWeight=FontWeight.Black,modifier=Modifier.weight(1f));IconButton(onClick=onStatus){Icon(Icons.Default.DonutLarge,null,tint=Accent)};IconButton(onClick=onContacts){Icon(Icons.Default.PersonAdd,null,tint=Color.White)};IconButton(onClick={}){Icon(Icons.Default.Search,null,tint=Color.White)}}
}

@Composable private fun BottomBar(tab:MainTab,onTab:(MainTab)->Unit){NavigationBar(containerColor=Color(0xFF071815),tonalElevation=0.dp){MainTab.entries.forEach{t->NavigationBarItem(selected=t==tab,onClick={onTab(t)},icon={Icon(t.icon,null)},label={Text(t.title,fontSize=10.sp)},colors=NavigationBarItemDefaults.colors(selectedIconColor=Bg,selectedTextColor=Accent,indicatorColor=Accent,unselectedIconColor=TextMuted,unselectedTextColor=TextMuted))}}}

@Composable private fun ChatsScreen(onChat:(Person)->Unit,onProfile:(Person)->Unit,onStatus:()->Unit){Column(Modifier.fillMaxSize().padding(horizontal=14.dp)){Row(Modifier.horizontalScroll(rememberScrollState()).padding(vertical=8.dp)){StoryAdd(onStatus);demoPeople.take(5).forEach{StoryChip(it,onProfile)}};SearchBox("",{},"ابحث في المحادثات...");LazyColumn(Modifier.fillMaxSize().padding(top=8.dp)){items(demoPeople){p->PersonRow(p,{onChat(p)},{onProfile(p)})}}}}
}

@Composable private fun CallsScreen(onProfile:(Person)->Unit){Column(Modifier.fillMaxSize().padding(horizontal=14.dp)){Text("سجل المكالمات",color=TextMuted,fontSize=12.sp,modifier=Modifier.padding(vertical=10.dp));LazyColumn{items(demoPeople.take(5)){p->CallRow(p,onProfile)}}}}
}

@Composable private fun CommunityScreen(onProfile:(Person)->Unit){Column(Modifier.fillMaxSize().padding(14.dp)){Surface(color=Accent.copy(.12f),shape=RoundedCornerShape(18.dp),modifier=Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp)){Text("مجتمعات شنو منو",color=Accent,fontWeight=FontWeight.Bold,fontSize=18.sp);Text("أنشئ مجتمعاً واجمع أصدقاءك ومجموعاتك في مكان واحد.",color=TextMuted,fontSize=12.sp);Spacer(Modifier.height(12.dp));Button(onClick={},colors=ButtonDefaults.buttonColors(containerColor=Accent,contentColor=Bg)){Icon(Icons.Default.Add,null);Text(" مجتمع جديد")}}};Spacer(Modifier.height(16.dp));Text("المجموعات",color=TextMuted);demoPeople.filter{it.name.contains("مجموعة")||it.name=="عائلتي"}.forEach{PersonRow(it,{onProfile(it)},{onProfile(it)})}}}
}

@Composable private fun SettingsScreen(){Column(Modifier.fillMaxSize().padding(16.dp)){Text("الحساب",color=Accent,fontWeight=FontWeight.Bold);Spacer(Modifier.height(8.dp));InfoCard(Icons.Default.Person,"الملف الشخصي","تعديل الاسم والصورة والمعلومات");InfoCard(Icons.Default.Notifications,"الإشعارات","نغمة المكالمات والرسائل");InfoCard(Icons.Default.Security,"الخصوصية","من يمكنه التواصل معك");InfoCard(Icons.Default.Storage,"التخزين","الوسائط والذاكرة المحلية");Spacer(Modifier.height(22.dp));OutlinedButton(onClick={},modifier=Modifier.fillMaxWidth()){Text("تسجيل الخروج",color=Danger)}}}

@Composable private fun ChatScreen(p:Person,onBack:()->Unit,onVoice:()->Unit,onVideo:()->Unit,onProfile:()->Unit){
    var msg by rememberSaveable{mutableStateOf("")}
    val sent = remember{mutableStateListOf("شلونك؟","تمام، الحمد لله. شنو الأخبار؟")}
    AppBackground{Column(Modifier.fillMaxSize().statusBarsPadding()){Row(Modifier.fillMaxWidth().padding(8.dp),verticalAlignment=Alignment.CenterVertically){IconButton(onClick=onBack){Icon(Icons.Default.ArrowBack,null,tint=Color.White)};Box(Modifier.clickable(onClick=onProfile)){Avatar(p.name,42.dp)};Column(Modifier.weight(1f).padding(horizontal=10.dp).clickable(onClick=onProfile)){Text(p.name,color=Color.White,fontWeight=FontWeight.Bold);Text(p.status,color=if(p.online)Accent else TextMuted,fontSize=10.sp)};IconButton(onClick=onVoice){Icon(Icons.Default.Call,null,tint=Accent)};IconButton(onClick=onVideo){Icon(Icons.Default.Videocam,null,tint=Accent)}};LazyColumn(Modifier.weight(1f).padding(12.dp)){items(sent){m->Surface(color=Card2,shape=RoundedCornerShape(16.dp),modifier=Modifier.padding(vertical=4.dp)){Text(m,color=Color.White,modifier=Modifier.padding(12.dp))}}};MessageComposer(msg,{msg=it}){if(msg.isNotBlank()){sent.add(msg);msg=""}}}}
}
