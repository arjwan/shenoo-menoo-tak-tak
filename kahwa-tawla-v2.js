...
function fallbackBoard(){
 ...
 [[0,'black',2],[11,'black',5],[16,'black',3],[18,'black',5],[23,'white',2],[12,'white',5],[7,'white',3],[5,'white',5]].forEach(function(x){b[x[0]][x[1]]=x[2]});
 return b
}
...
function point(pub,n,top){
 var p=pub.board&&pub.board[n]||{white:0,black:0},from=actions().some(a=>a.type==='move'&&a.from===n),to=selected!==null&&actions().some(a=>a.type==='move'&&a.from===selected&&a.to===n);
 return '<button type="button" aria-label="خانة '+(n+1)+'" class="tawla-point '+(top?'top ':'bottom ')+(from?'can-from ':'')+(to?'can-to ':'')+(selected===n?'selected':'')+'" data-point="'+n+'"><span>'+(n+1)+'</span><div>'+checker('white',p.white||0)+checker('black',p.black||0)+'</div></button>'
}
...
function render(c,x){
 ...
 c.querySelectorAll('[data-point]').forEach(function(el){
  ...
  el.onpointerdown=function(e){if(el.classList.contains('can-from')||el.classList.contains('can-to'))el.classList.add('is-touched');el.setPointerCapture&&el.setPointerCapture(e.pointerId)};
  el.onpointerup=function(e){e.preventDefault();el.classList.remove('is-touched');pick(Number(el.dataset.point),c)};
  el.onpointercancel=function(){el.classList.remove('is-touched')};
 });
