...
router.post('/', requireAuth, (req,res,next)=>upload.fields([{name:'logo',maxCount:1},{name:'cover',maxCount:1}])(req,res,err=>err?res.status(err.status||400).json({ok:false,message:err.message||'تعذر رفع الصور'}):next()), async (req,res)=>{
  try {
    const { name,category,description,governorate,area,address,phone,whatsapp,facebook,instagram,website,plan,delegate1Name,delegate1Phone,delegate2Name,delegate2Phone }=req.body;
    ...
    const store=await Store.create({ owner:req.user._id,name:clean(name,120),category:clean(category,120),description:clean(description,1000),governorate:clean(governorate,80),area:clean(area,100),address:clean(address,240),phone:clean(phone,30),whatsapp:clean(whatsapp,30),facebook:clean(facebook,220),instagram:clean(instagram,220),website:clean(website,220),plan,logoUrl:files.logo?.[0]?`/uploads/stores/${files.logo[0].filename}`:'',coverUrl:files.cover?.[0]?`/uploads/stores/${files.cover[0].filename}`:'',delegates,status:'pending' });
...
router.get('/mine',requireAuth,async(req,res)=>{const stores=await Store.find({owner:req.user._id}).sort({createdAt:-1}).lean();res.json({ok:true,stores:stores.map(s=>({id:s._id,name:s.name,category:s.category,plan:s.plan,status:s.status,createdAt:s.createdAt}))});});
router.get('/',async(req,res)=>{const stores=await Store.find({status:'approved'}).sort({createdAt:-1}).select('name category description governorate area logoUrl coverUrl plan').lean();res.json({ok:true,stores});});
