...
function storyView(s){return {id:s._id,author:authorView(s.author),text:s.text,media:s.media||[],overlays:s.overlays||[],music:s.music||{},createdAt:s.createdAt,expiresAt:s.expiresAt};}
...
router.get('/',async(req,res)=>{try{const friends=(await FriendRequest.find({$or:[{sender:req.user._id},{receiver:req.user._id}],status:'accepted'}).select('sender receiver')).map(r=>String(r.sender)===String(req.user._id)?r.receiver:r.sender);const stories=await Story.find({author:{$in:[req.user._id,...friends]},active:true,expiresAt:{$gt:new Date()}}).populate('author','fullName displayName username profile').sort({createdAt:-1}).limit(50);res.json({ok:true,stories:stories.map(storyView)});}catch(e){res.status(500).json({ok:false,message:e.message});}});
