...
const MAX_REEL_BYTES=1024*1024*1024;
const types=new Map([
 ['video/mp4','.mp4'],['video/x-m4v','.m4v'],['video/webm','.webm'],['video/quicktime','.mov'],['video/ogg','.ogv'],['video/3gpp','.3gp'],['video/3gpp2','.3g2'],['video/x-matroska','.mkv'],['video/x-msvideo','.avi'],['video/mpeg','.mpeg'],['video/mp2t','.ts'],['video/x-flv','.flv'],['video/x-ms-wmv','.wmv'],['video/x-ms-asf','.asf'],['video/x-f4v','.f4v'],
 ['audio/mpeg','.mp3'],['audio/mp3','.mp3'],['audio/mp4','.m4a'],['audio/x-m4a','.m4a'],['audio/aac','.aac'],['audio/wav','.wav'],['audio/x-wav','.wav'],['audio/flac','.flac'],['audio/x-flac','.flac'],['audio/ogg','.ogg'],['audio/opus','.opus'],['audio/amr','.amr'],['audio/webm','.webm']
]);
const allowedExt=new Set(['.mp4','.m4v','.mov','.webm','.ogv','.ogg','.3gp','.3g2','.mkv','.avi','.mpeg','.mpg','.ts','.mts','.m2ts','.flv','.wmv','.asf','.vob','.f4v','.ogm','.mp3','.m4a','.aac','.wav','.flac','.opus','.oga','.amr']);
function mediaExt(file){const ext=path.extname(file.originalname||'').toLowerCase();return types.get(file.mimetype)||(allowedExt.has(ext)?ext:'');}
const upload=multer({storage:multer.diskStorage({destination:(_r,_f,cb)=>cb(null,uploadDir),filename:(_r,f,cb)=>cb(null,`${Date.now()}-${Math.random().toString(36).slice(2)}${mediaExt(f)}`)}),limits:{fileSize:MAX_REEL_BYTES,files:1},fileFilter:(_r,f,cb)=>mediaExt(f)?cb(null,true):cb(new multer.MulterError('LIMIT_UNEXPECTED_FILE',f.fieldname))});
...
async function normalizeMedia(file){
 if(!file)return file;
 const isAudio=String(file.mimetype||'').startsWith('audio/')||['.mp3','.m4a','.aac','.wav','.flac','.opus','.oga','.amr'].includes(path.extname(file.originalname||'').toLowerCase());
 if((isAudio&&file.mimetype==='audio/mpeg')||(!isAudio&&['video/mp4','video/webm'].includes(file.mimetype)))return{...file,mediaType:isAudio?'audio':'video'};
 const output=file.path.replace(/\.[^.]+$/, '')+(isAudio?'-converted.mp3':'-converted.mp4');
 const args=isAudio?['-y','-i',file.path,'-vn','-c:a','libmp3lame','-b:a','192k',output]:['-y','-i',file.path,'-vf','scale=min(1280\\,iw):-2','-c:v','libx264','-preset','veryfast','-crf','24','-c:a','aac','-b:a','128k','-movflags','+faststart',output];
 try{await run('ffmpeg',args,{timeout:60*60*1000,maxBuffer:4*1024*1024});const stat=await fs.promises.stat(output);await fs.promises.unlink(file.path);return{...file,path:output,filename:path.basename(output),mimetype:isAudio?'audio/mpeg':'video/mp4',mediaType:isAudio?'audio':'video',size:stat.size};}catch(e){await fs.promises.unlink(output).catch(()=>{});throw Error('تعذر تحويل الملف. تأكد من سلامة الفيديو أو الصوت ومن توفر FFmpeg على الخادم.');}
}
