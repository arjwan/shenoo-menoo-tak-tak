(function () {
  ...
  function showToast(text, type) {
    ...
    toast.className = "settings-toast is-visible " + (type || "success");
    clearTimeout(toastTimer);
    toastTimer = setTimeout(function () { toast.className = "settings-toast"; }, 3200);
  }
  function message(form, text, type) {
    var target = form.querySelector("[data-message]") || document.querySelector("[data-security-message]");
    if (target) { target.textContent = text; target.className = "settings-message " + (type || ""); }
  }
  function feedback(form, text, type) { message(form, text, type); showToast(text, type); }
  function guarded(form) {
    if (isOffline()) { feedback(form, "يجب الاتصال بالإنترنت لحفظ هذا التغيير", "error"); return true; }
    return false;
  }
  ...
  async function submit(form) {
    ...
    if (form.dataset.form === "contact") {
      ...
      var phoneValue = form.elements.phone.value.trim();
      var contactField = emailValue ? "email" : "phone";
      var contactValue = contactField === "email" ? emailValue : phoneValue;
      if (!contactValue) { feedback(form, "أدخل البريد أو الهاتف أولاً", "error"); return; }
    ...
    if (form.dataset.form === "profile") {
      if (form.elements.avatar.files.length || form.elements.cover.files.length) { feedback(form, "رفع الصور يحتاج نقطة رفع صور موثقة من الخادم، ولم يتم حفظ الملف.", "error"); return; }
      payload.website = String(payload.website || "").trim();
      payload.socialLinks = [];
    }
    if (form.dataset.form === "password" && payload.newPassword !== payload.confirmPassword) { feedback(form, "كلمتا المرور الجديدتان غير متطابقتين", "error"); return; }
    if (button) { button.disabled = true; button.classList.add("is-saving"); button.textContent = "جارٍ الحفظ…"; }
    message(form, "جارٍ حفظ التغييرات…", "");
    try {
      var data = await SocialAPI.request(endpoint, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      var successText = data.message || "تم حفظ التغييرات بنجاح";
      feedback(form, "✓ " + successText, "success");
      if (button) button.textContent = "✓ تم الحفظ";
      await load();
      if (form.dataset.form === "password") form.reset();
      setTimeout(function () { if (button) { button.textContent = originalButtonText; button.disabled = false; button.classList.remove("is-saving"); } }, 1800);
    } catch (error) {
      feedback(form, error.message || "تعذر حفظ التغييرات", "error");
      if (button) { button.textContent = originalButtonText; button.disabled = false; button.classList.remove("is-saving"); }
    }
  }
  ...
  document.querySelector("[data-blocked-list]").addEventListener("click", async function (event) { var button = event.target.closest("[data-unblock]"); if (!button || isOffline()) return; try { await SocialAPI.request("/api/users/" + button.dataset.unblock + "/block", { method: "DELETE" }); showToast("تم فك الحظر", "success"); blocked(); } catch (error) { button.textContent = error.message; } });
